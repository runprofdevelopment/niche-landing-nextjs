...
export default function EventTimelineView({ eventId }: { eventId: string }) {
  const t = useTranslations("events");
  const { can } = usePermissions();
  const { event, loading } = useEventQuery(eventId);
  const { halls } = useEventHallListQuery(eventId);
  const canView = can(EVENT_PERMISSIONS.timelineView);
  ...
  if (!canView) {
    return (
      <div className="flex min-h-[40vh] flex-col items-center justify-center gap-4 text-center">
        <p className="text-muted-foreground">{t("eventTimelineNoPermission")}</p>
        <Button variant="outline" asChild>
  ...
  return (
    <PermissionGate permission={EVENT_PERMISSIONS.timelineView}>
      <div className="space-y-6">
        <EventSummaryHeader event={event} halls={halls} editHref={routes.event(eventId)} />
        <EventTimeline eventId={eventId} eventDate={event.date} />
      </div>
    </PermissionGate>
  );
}
